...
pub fn algae_inject(input: TokenStream) -> TokenStream {
    ...
    let argument_names: Vec<Ident> = signature
        .arguments
        .iter()
        .map(|arg| arg.name.clone())
        .collect();
    //Wrapp all function arguments into the wrapper struct
    let wrapper: Vec<Wrapper> = signature
        .arguments
        .into_iter()
        .map(|arg| Wrapper::from(arg))
        .collect();
    //Builds call signature
    let wrapper_signature: Vec<WrappedFunctionSignature> = wrapper
        .iter()
        .map(|w| WrappedFunctionSignature::from(w))
        .collect();
